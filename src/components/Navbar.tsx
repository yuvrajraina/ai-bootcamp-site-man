import { navItems, heroContent, buildWhatsApp } from "@/lib/content";

export default function Navbar() {
  return (
    <header className="sticky top-0 z-50 border-b border-white/40 bg-white/70 backdrop-blur">
      <div className="mx-auto flex max-w-6xl items-center justify-between px-4 py-3 sm:px-6 lg:px-8">
        <div className="text-lg font-semibold tracking-tight text-ink">AI Bootcamp</div>
        <nav
        ...
        >
          {navItems.map((item) => (
            <a
              key={item.href}
              href={item.href}
              className="rounded transition hover:text-ink focus-ring"
            >
              {item.label}
            </a>
          ))}
